import Business, { BusinessModal, IBusinessQuestions } from './businessAnswers';
...
export default class BusinessRepo {
  ...
  public static async updateBusiness(id: string, body: Business): Promise<{ settingData: any }> {
    const settingData = await BusinessModal.findOneAndUpdate({ business: id }, { ...body } as Business, { new: true });
    if (!settingData) throw new NoDataError();
    return { settingData };
  }
