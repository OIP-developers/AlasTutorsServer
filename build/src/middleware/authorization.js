...
function authorization(allow_roles) {
    return (0, asyncHandler_1.default)((req, _, next) => __awaiter(this, void 0, void 0, function* () {
        const role = yield role_repository_1.default.findById(req.user.roleId);
        if (!allow_roles.includes(req.user.role.code))
            throw new ApiError_1.AuthFailureError("Permission denied, You don't have permission for this action");
        else if (!req.user || !req.user.role)
            throw new ApiError_1.AuthFailureError("Permission denied, You don't have role");
        else if (!role)
